...
void run_set_chunk_data(Chunk* chunk, Settings* settings)
{
    ...
    set_chunk_data(
            ...
            *(chunk->x_area), *(chunk->y_area), x_min, y_min,
            settings->dx, settings->dy, *(device_queue));
...
void run_set_chunk_state(Chunk* chunk, Settings* settings, State* states)
{
    ...
    set_chunk_initial_state(
            ...
            states[0].density, *(chunk->energy0), *(chunk->density), *(device_queue));
        ...
        int face, bool pack, SyclBuffer* field, double* buffer)
{
    ...
    {
      {//scope?
        //auto hostAcc = *(chunk->ext->comms_buffer).get_access<access::mode::write, access::host_buffer>(); //need to destroy
...
#define PACK_HALO(face_val, pack_val, pack_kernel) \
  ...
  if(pack)
  {
    { //TODO scope destroys accessor??
      //auto hostAcc = *(chunk->ext->comms_buffer).get_access<access::mode::read, access::host_buffer>(); //need to destroy
      //unpack copies contents from host accessor to buffer
