...
// Initialises p,r,u,w
void cg_init_u(
  const int x, const int y, const int coefficient, SyclBuffer& pBuff,
  SyclBuffer& rBuff, SyclBuffer& uBuff, SyclBuffer& wBuff,
...
void cg_calc_w(
  const int x, const int y, const int halo_depth, SyclBuffer& wBuff,
  SyclBuffer& pBuff, SyclBuffer& kxBuff, SyclBuffer& kyBuff, double* pw,
  queue& device_queue)
{
  size_t wgroup_size = WORK_GROUP_SIZE;
